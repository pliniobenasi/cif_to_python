...
class SimpleEnvironmentModule(AbstractCropModule):
    ...
    def __init__(self, name: str = "SimpleEnvironmentModule", granularity: int = 3600, params: dict = {}) -> None:
        ...
        Optional spatial parameters:
            - row: plant row in the grid
            - col: plant column in the grid
        ...
        # Spatial position of the plant in the grid
        self.row = int(params.get("row", 0))
        self.col = int(params.get("col", 0))
        self.rows = int(params.get("rows", 1))
        self.cols = int(params.get("cols", 1))
    ...
    def change_params(self, params: dict) -> None:
        ...
        if isinstance(params["Initial_temperature"], (Decimal, int)):
            ...
            self.params["Initial_temperature"] = Decimal(str(params["Initial_temperature"]))

        self.params["Initial_solar_radiation"] = (
            Decimal(params["Initial_solar_radiation"])
            if isinstance(params["Initial_solar_radiation"], (Decimal, int))
            else Decimal(str(params["Initial_solar_radiation"]))
        )
        ...
        # Optional spatial parameters
        self.row = int(params.get("row", getattr(self, "row", 0)))
        self.col = int(params.get("col", getattr(self, "col", 0)))
        self.rows = int(params.get("rows", getattr(self, "rows", 1)))
        self.cols = int(params.get("cols", getattr(self, "cols", 1)))
    ...
    def step(self, input: list) -> None:
        ...
        # Base weather values from input file
        ...
        # Spatial modifier for solar radiation.
        # Radiation decreases from west to east.
        if self.cols > 1:
            radiation_modifier = Decimal("1.0") - Decimal("0.1") * Decimal(self.col) / Decimal(self.cols - 1)
        else:
            radiation_modifier = Decimal("1.0")

        # Spatial modifier for temperature.
        # Temperature increases from north to south.
        if self.rows > 1:
            temperature_modifier = Decimal("0.15") * Decimal(self.row) / Decimal(self.rows - 1)
        else:
            temperature_modifier = Decimal("0.0")
        ...
        """        
        # Apply spatial variability.
        # This keeps the original temporal processing below unchanged.
        sr = sr * radiation_modifier
        temp = temp + sr * temperature_modifier / Decimal("300")
        
        #check
        """
        if self.name.endswith("P0_2") or self.name.endswith("P2_2"):
             print(
                   self.name,
                   "row:", self.row,
                   "col:", self.col,
                   "rad_modifier:", radiation_modifier,
                   "temp_modifier:", temperature_modifier,
             )
		"""
        ...
        self.PPFD = self.state_variables["PPFD"]
        self.Temperature = self.state_variables["Temperature"]
        self.Solar_radiation = self.state_variables["Solar_radiation"]

        self.state_variables["Hour"] = hour

        self.daily_temperature_sum += temp
        self.state_variables["Daily_Average_Temperature"] = (
            self.daily_temperature_sum / (Decimal(hour) + 1)
        )

        if hour >= 6 and hour <= 18: # Assuming daytime is from 6 AM to 6 PM
            self.daily_daytime_temperature_sum += temp
            self.state_variables["Daily_Daytime_Average_Temperature"] = (
                self.daily_daytime_temperature_sum / (Decimal(hour) - 5)
            ) # Number of daytime hours passed
