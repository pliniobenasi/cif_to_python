...
"""
Utilities for generated package usability.

The pipeline already generates Python modules plus feature/manifest reports.
This module adds practical artifacts inside each generated output folder:

- source_model.cif
- run_generated_model.py
- README_GENERATED_MODEL.md

The generated runner is generic: it delegates execution to auto_builder.py and
therefore works for both step-driven models with generated input modules and
event-driven models driven by explicit events.
"""
...
BAKED_PIPELINE_ROOT = Path("__PIPELINE_ROOT__")
...
def copy_source_cif(cif_path: str | Path, generated_package_dir: str | Path) -> Path:
    cif_path = Path(cif_path)
    generated_package_dir = Path(generated_package_dir)
    target = generated_package_dir / "source_model.cif"
    shutil.copy(cif_path, target)
    return target


def write_generated_runner(generated_package_dir: str | Path, pipeline_root: str | Path | None = None) -> Path:
    generated_package_dir = Path(generated_package_dir)
    runner_path = generated_package_dir / "run_generated_model.py"
    pipeline_root = Path(pipeline_root).resolve() if pipeline_root is not None else Path.cwd().resolve()
    runner_path.write_text(RUNNER_CODE.replace("__PIPELINE_ROOT__", str(pipeline_root)))
    runner_path.chmod(0o755)
    return runner_path


def write_generated_readme(generated_package_dir: str | Path) -> Path:
    generated_package_dir = Path(generated_package_dir)
    readme_path = generated_package_dir / "README_GENERATED_MODEL.md"
    ...
    readme = textwrap.dedent(
        ...
        *.py generated modules
        ...
        ```bash
        python3 run_generated_model.py --input-csv ../examples/grid_weather_input.csv \
          --limit 10 \
          --save-every 1 \
          --history-csv ../../outputs/Grid/results/grid_history.csv
        ```
...
def write_generated_artifacts(cif_path: str | Path, generated_package_dir: str | Path, pipeline_root: str | Path | None = None) -> dict[str, str]:
    source = copy_source_cif(cif_path, generated_package_dir)
    runner = write_generated_runner(generated_package_dir, pipeline_root=pipeline_root)
    readme = write_generated_readme(generated_package_dir)
